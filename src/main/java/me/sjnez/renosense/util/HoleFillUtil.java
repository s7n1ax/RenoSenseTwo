...
public class HoleFillUtil {
    ...
    public static void placeBlockScaffold(BlockPos pos) {
        ...
        for (EnumFacing side : EnumFacing.values()) {
            ...
            if (!HoleFillUtil.canBeClicked(neighbor) || !(eyesPos.squareDistanceTo(hitVec = new Vec3d(neighbor).addVector(0.5, 0.5, 0.5).add(new Vec3d(side2.getDirectionVec()).scale(0.5))) <= 18.0625)) continue;
    ...
    public static EnumFacing getPlaceableSide(BlockPos pos) {
        for (EnumFacing side : EnumFacing.values()) {
            ...
            if (!HoleFillUtil.mc.world.getBlockState(neighbour).getBlock().canCollideCheck(HoleFillUtil.mc.world.getBlockState(neighbour), false) || (blockState = HoleFillUtil.mc.world.getBlockState(neighbour)).func_185904_a().isReplaceable()) continue;
            return side;
        }
        return null;
    }
    ...
    public static boolean hasNeighbour(BlockPos blockPos) {
        for (EnumFacing side : EnumFacing.values()) {
            BlockPos neighbour = blockPos.offset(side);
            if (HoleFillUtil.mc.world.getBlockState(neighbour).func_185904_a().isReplaceable()) continue;
            return true;
        }
        return false;
    }
