...
import me.sjnez.renosense.util.HoleFillUtil;
...
implements Util {
    ...
    public static List<EnumFacing> getPossibleSides(BlockPos pos) {
        ...
        for (EnumFacing side : EnumFacing.values()) {
            ...
            if (!BlockUtil.mc.world.getBlockState(neighbour).getBlock().canCollideCheck(BlockUtil.mc.world.getBlockState(neighbour), false) || (blockState = BlockUtil.mc.world.getBlockState(neighbour)).func_185904_a().isReplaceable()) continue;
            facings.add(side);
        }
        return facings;
    }
    ...
    private static boolean hasNeighbour(BlockPos blockPos) {
        for (EnumFacing side : EnumFacing.values()) {
            BlockPos neighbour = blockPos.offset(side);
            if (BlockUtil.mc.world.getBlockState(neighbour).func_185904_a().isReplaceable()) continue;
            return true;
        }
        return false;
    }
