...
implements Util {
    ...
    public static boolean canTakeDamage(boolean suicide) {
        return !DamageUtil.mc.player.field_71075_bZ.isCreativeMode && !suicide;
    }
    ...
    public static float getBlastReduction(EntityLivingBase entity, float damageI, Explosion explosion) {
        ...
        if (entity instanceof EntityPlayer) {
            ...
            damage = CombatRules.getDamageAfterAbsorb((float)damage, (float)ep.func_70658_aO(), (float)((float)ep.func_110148_a(SharedMonsterAttributes.ARMOR_TOUGHNESS).getAttributeValue()));
            int k = 0;
            try {
    ...
    public static float getDamageMultiplied(float damage) {
        int diff = DamageUtil.mc.world.func_175659_aa().getDifficultyId();
