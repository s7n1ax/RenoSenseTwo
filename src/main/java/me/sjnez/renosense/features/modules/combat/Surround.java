...
extends Module {
    ...
    public Surround() {
        super("Surround", "Surrounds your feet with Obsidian.", Module.Category.COMBAT, true, false, false);
    }
    ...
    public void onEnable() {
        ...
        if (this.center.getValue().booleanValue()) {
            RenoSense.positionManager.setPositionPacket((double)this.startPos.func_177958_n() + 0.5, this.startPos.func_177956_o(), (double)this.startPos.func_177952_p() + 0.5, true, true, true);
        }
        this.retries.clear();
    ...
    private void doFeetPlace() {
        ...
        if (!EntityUtil.isSafe(Surround.mc.player, 0, true)) {
            this.isSafe = 0;
            this.placeBlocks(Surround.mc.player.func_174791_d(), EntityUtil.getUnsafeBlockArray(Surround.mc.player, 0, true), true, false, false);
        ...
        block5: for (Vec3d vec3d : vec3ds) {
            ...
            switch (BlockUtil.isPositionPlaceable(position, false)) {
                case 1: {
                    ...
                    this.placeBlocks(Surround.mc.player.func_174791_d().add(vec3d), EntityUtil.getUnsafeBlockArrayFromVec3d(Surround.mc.player.func_174791_d().add(vec3d), 0, true), hasHelpingBlocks, false, true);
    ...
    private void placeBlock(BlockPos pos) {
        if (this.placements < this.blocksPerTick.getValue()) {
            int originalSlot = Surround.mc.player.field_71071_by.currentItem;
            int obbySlot = InventoryUtil.findHotbarBlock(BlockObsidian.class);
            int eChestSot = InventoryUtil.findHotbarBlock(BlockEnderChest.class);
            if (obbySlot == -1 && eChestSot == -1) {
                this.toggle();
            ...
            isPlacing = true;
            Surround.mc.player.field_71071_by.currentItem = obbySlot == -1 ? eChestSot : obbySlot;
            ...
            Surround.mc.player.field_71071_by.currentItem = originalSlot;
            Surround.mc.playerController.updateController();
            this.didPlace = true;
            ++this.placements;
        }
