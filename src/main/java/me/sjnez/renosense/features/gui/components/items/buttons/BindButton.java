...
extends Button {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.isListening) {
            RenoSense.textManager.drawStringWithShadow("Press a Key...", this.x + 2.3f, this.y - 1.7f - (float)RenoSenseGui.getClickGui().getTextOffset(), -1);
        } else {
            RenoSense.textManager.drawStringWithShadow(this.setting.getName() + " " + ChatFormatting.RED + this.setting.getValue().toString().toUpperCase(), this.x + 2.3f, this.y - 1.7f - (float)RenoSenseGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        }
    }
