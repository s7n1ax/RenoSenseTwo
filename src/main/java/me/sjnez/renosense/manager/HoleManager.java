...
extends Feature {
    ...
    public List<BlockPos> getSortedHoles() {
        this.holes.sort(Comparator.comparingDouble(hole -> HoleManager.mc.player.func_174818_b((BlockPos)hole)));
    ...
    public List<BlockPos> calcHoles() {
        ...
        for (BlockPos pos : positions) {
            if (!HoleManager.mc.world.func_180495_p(pos).getBlock().equals(Blocks.AIR) || !HoleManager.mc.world.func_180495_p(pos.add(0, 1, 0)).getBlock().equals(Blocks.AIR) || !HoleManager.mc.world.func_180495_p(pos.add(0, 2, 0)).getBlock().equals(Blocks.AIR)) continue;
            boolean isSafe = true;
            boolean midSafe = true;
            for (BlockPos offset : surroundOffset) {
                Block block = HoleManager.mc.world.func_180495_p(pos.add(offset)).getBlock();
