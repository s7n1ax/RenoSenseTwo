...
extends Module {
    ...
    public void fogColors(EntityViewRenderEvent.FogColors event) {
        if (this.colorSync.getValue().booleanValue()) {
            event.setRed((float)ClickGui.getInstance().red.getValue().intValue() / 255.0f);
        } else {
            event.setRed((float)this.red.getValue().intValue() / 255.0f);
        }
        if (this.colorSync.getValue().booleanValue()) {
            event.setGreen((float)ClickGui.getInstance().green.getValue().intValue() / 255.0f);
        } else {
            event.setGreen((float)this.green.getValue().intValue() / 255.0f);
        }
        if (this.colorSync.getValue().booleanValue()) {
            event.setBlue((float)ClickGui.getInstance().blue.getValue().intValue() / 255.0f);
        } else {
