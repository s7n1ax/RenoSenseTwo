...
implements Util {
    ...
    public static void gradientBox(BlockPos pos, Color color, float lineWidth, boolean outline, boolean box, int boxAlpha, boolean speedmine) {
        ...
        if (camera.isBoundingBoxInFrustum(new AxisAlignedBB(bb.minX + RenderUtil.mc.getRenderManager().viewerPosX, bb.minY + RenderUtil.mc.getRenderManager().viewerPosY, bb.minZ + RenderUtil.mc.getRenderManager().viewerPosZ, bb.maxX + RenderUtil.mc.getRenderManager().viewerPosX, bb.maxY + RenderUtil.mc.getRenderManager().viewerPosY, bb.maxZ + RenderUtil.mc.getRenderManager().viewerPosZ))) {
            ...
            double percentage = RenderUtil.mc.playerController.curBlockDamageMP;
            float speedmineThing = (float)PacketMine.getInstance().timer.getPassedTimeMs() / 1000.0f / (PacketMine.getInstance().breakTime * RenoSense.serverManager.getTpsFactor());
            float f = speedmineThing = speedmineThing > 1.0f ? 1.0f : speedmineThing;
            if (speedmine) {
                minX = bb.minX + 0.5 - (double)(speedmineThing / 2.0f);
                minY = bb.minY + 0.5 - (double)(speedmineThing / 2.0f);
                minZ = bb.minZ + 0.5 - (double)(speedmineThing / 2.0f);
                maxX = bb.maxX - 0.5 + (double)(speedmineThing / 2.0f);
                maxY = bb.maxY - 0.5 + (double)(speedmineThing / 2.0f);
                maxZ = bb.maxZ - 0.5 + (double)(speedmineThing / 2.0f);
            } else {
            ...
            AxisAlignedBB a = new AxisAlignedBB(minX, minY, minZ, maxX, maxY, maxZ);
            if (box) {
                RenderUtil.drawFilledBox(a, new Color((float)color.getRed() / 255.0f, (float)color.getGreen() / 255.0f, (float)color.getBlue() / 255.0f, (float)boxAlpha / 255.0f).getRGB());
            }
            if (outline) {
                RenderUtil.drawBlockOutline(a, new Color((float)color.getRed() / 255.0f, (float)color.getGreen() / 255.0f, (float)color.getBlue() / 255.0f, 1.0f), PacketMine.getInstance().lineWidth.getValue().floatValue());
            }
            GL11.glDisable(2848);
            GlStateManager.depthMask((boolean)true);
            GlStateManager.enableDepth();
    ...
    public void drawText(BlockPos pos, String text, Color color) {
        ...
        GlStateManager.disableDepth();
        GlStateManager.translate((double)(-((double)RenoSense.textManager.getStringWidth(text) / 2.0)), (double)0.0, (double)0.0);
    ...
    public static void drawBlockOutline(BlockPos pos, Color color, float linewidth, boolean air, double height, boolean gradient, boolean invert, int alpha) {
        ...
        IBlockState iblockstate = RenderUtil.mc.world.getBlockState(pos);
        if ((air || iblockstate.func_185904_a() != Material.AIR) && RenderUtil.mc.world.getWorldBorder().contains(pos)) {
    ...
    public static void drawOutlinedBlockESP(BlockPos pos, Color color, float linewidth) {
        IBlockState iblockstate = RenderUtil.mc.world.getBlockState(pos);
        Vec3d interp = EntityUtil.interpolateEntity(RenderUtil.mc.player, mc.getRenderPartialTicks());
        RenderUtil.drawBoundingBox(iblockstate.func_185918_c(RenderUtil.mc.world, pos).grow(0.002f).offset(-interp.x, -interp.y, -interp.z), linewidth, ColorUtil.toRGBA(color));
    }
    ...
    public static AxisAlignedBB getBoundingBox(BlockPos blockPos) {
        return RenderUtil.mc.world.getBlockState(blockPos).func_185900_c(RenderUtil.mc.world, blockPos).offset(blockPos);
    }
    ...
    extends Tessellator {
        ...
        public static void drawBox(BlockPos blockPos, int r, int g, int b, int a, int sides) {
            RenderTesselator.drawBox(INSTANCE.getBuffer(), blockPos.func_177958_n(), blockPos.func_177956_o(), blockPos.func_177952_p(), 1.0f, 1.0f, 1.0f, r, g, b, a, sides);
        }
        ...
        public static void drawHalfBox(BlockPos blockPos, int r, int g, int b, int a, int sides) {
            RenderTesselator.drawBox(INSTANCE.getBuffer(), blockPos.func_177958_n(), blockPos.func_177956_o(), blockPos.func_177952_p(), 1.0f, 0.5f, 1.0f, r, g, b, a, sides);
        }
    }
