...
extends Button {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.isListening) {
            RenoSense.textManager.drawStringWithShadow(this.currentString.getString() + RenoSense.textManager.getIdleSign(), this.x + 2.3f, this.y - 1.7f - (float)RenoSenseGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        } else {
            RenoSense.textManager.drawStringWithShadow((this.setting.getName().equals("Buttons") ? "Buttons " : (this.setting.getName().equals("Prefix") ? "Prefix  " + ChatFormatting.RED : "")) + this.setting.getValue(), this.x + 2.3f, this.y - 1.7f - (float)RenoSenseGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        }
    }
