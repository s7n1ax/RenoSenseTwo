...
implements Util {
    ...
    public static List<Vec3d> getUnsafeBlocksFromVec3d(Vec3d pos, int height, boolean floor) {
        ...
        for (Vec3d vector : EntityUtil.getOffsets(height, floor)) {
            ...
            Block block = EntityUtil.mc.world.func_180495_p(targetPos).getBlock();
    ...
    public static boolean isObbyHole(BlockPos blockPos) {
        ...
        for (BlockPos pos : touchingBlocks) {
            IBlockState touchingState = EntityUtil.mc.world.func_180495_p(pos);
    ...
    public static double getDst(Vec3d vec) {
        return EntityUtil.mc.player.func_174791_d().distanceTo(vec);
    }
    ...
    public static boolean isInWater(Entity entity) {
        ...
        for (int x = MathHelper.floor((double)entity.posX); x < MathHelper.ceil((double)entity.posX); ++x) {
            for (int z = MathHelper.floor((double)entity.posZ); z < MathHelper.ceil((double)entity.posZ); ++z) {
                ...
                if (!(EntityUtil.mc.world.func_180495_p(pos).getBlock() instanceof BlockLiquid)) continue;
                return true;
            }
        }
        return false;
    ...
    public static boolean isDrivenByPlayer(Entity entityIn) {
        return EntityUtil.mc.player != null && entityIn != null && entityIn.equals(EntityUtil.mc.player.func_184187_bx());
    }
    ...
    public static boolean canEntityFeetBeSeen(Entity entityIn) {
        return EntityUtil.mc.world.func_147447_a(new Vec3d(EntityUtil.mc.player.field_70165_t, EntityUtil.mc.player.field_70165_t + (double)EntityUtil.mc.player.func_70047_e(), EntityUtil.mc.player.field_70161_v), new Vec3d(entityIn.posX, entityIn.posY, entityIn.posZ), false, true, false) == null;
    }

    public static boolean isntValid(Entity entity, double range) {
        return entity == null || EntityUtil.isDead(entity) || entity.equals(EntityUtil.mc.player) || entity instanceof EntityPlayer && RenoSense.friendManager.isFriend(entity.getName()) || EntityUtil.mc.player.func_70068_e(entity) > MathUtil.square(range);
    }
    ...
    public static double getMaxSpeed() {
        ...
        if (EntityUtil.mc.player.func_70644_a(Objects.requireNonNull(Potion.getPotionById((int)1)))) {
            maxModifier *= 1.0 + 0.2 * (double)(Objects.requireNonNull(EntityUtil.mc.player.func_70660_b(Objects.requireNonNull(Potion.getPotionById((int)1)))).getAmplifier() + 1);
        }
        return maxModifier;
    }
    ...
    public static void moveEntityStrafe(double speed, Entity entity) {
        if (entity != null) {
            ...
            float yaw = EntityUtil.mc.player.field_70177_z;
    ...
    public static boolean rayTraceHitCheck(Entity entity, boolean shouldCheck) {
        return !shouldCheck || EntityUtil.mc.player.func_70685_l(entity);
    }
    ...
    public static boolean isMoving() {
        return (double)EntityUtil.mc.player.field_191988_bg != 0.0 || (double)EntityUtil.mc.player.field_70702_br != 0.0;
    }

    public static EntityPlayer getClosestEnemy(double distance) {
        EntityPlayer closest = null;
        for (EntityPlayer player : EntityUtil.mc.world.field_73010_i) {
            if (EntityUtil.isntValid(player, distance)) continue;
            if (closest == null) {
                closest = player;
                continue;
            }
            if (!(EntityUtil.mc.player.func_70068_e(player) < EntityUtil.mc.player.func_70068_e(closest))) continue;
            closest = player;
        }
        return closest;
    }
    ...
    public static boolean checkCollide() {
        ...
        if (EntityUtil.mc.player.func_184187_bx() != null && EntityUtil.mc.player.func_184187_bx().fallDistance >= 3.0f) {
            return false;
        }
        return EntityUtil.mc.player.field_70143_R < 3.0f;
    }

    public static BlockPos getPlayerPosWithEntity() {
        return new BlockPos(EntityUtil.mc.player.func_184187_bx() != null ? EntityUtil.mc.player.func_184187_bx().posX : EntityUtil.mc.player.field_70165_t, EntityUtil.mc.player.func_184187_bx() != null ? EntityUtil.mc.player.func_184187_bx().posY : EntityUtil.mc.player.field_70163_u, EntityUtil.mc.player.func_184187_bx() != null ? EntityUtil.mc.player.func_184187_bx().posZ : EntityUtil.mc.player.field_70161_v);
    }
    ...
    public static double[] forward(double speed) {
        ...
        float yaw = EntityUtil.mc.player.field_70126_B + (EntityUtil.mc.player.field_70177_z - EntityUtil.mc.player.field_70126_B) * mc.getRenderPartialTicks();
        if (forward != 0.0f) {
            if (side > 0.0f) {
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (EntityPlayer player : EntityUtil.mc.world.field_73010_i) {
            if (player.func_82150_aj() || player.getName().equals(EntityUtil.mc.player.func_70005_c_())) continue;
            ...
            healthSB.append(hp);
            int distanceInt = (int)EntityUtil.mc.player.func_70032_d(player);
            ...
            output.put(healthSB.toString() + " " + (RenoSense.friendManager.isFriend(player) ? ChatFormatting.AQUA : ChatFormatting.RED) + player.getName() + " " + distanceSB.toString() + " Â§f0", (int)EntityUtil.mc.player.func_70032_d(player));
